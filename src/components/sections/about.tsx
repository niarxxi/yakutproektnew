...
const STLLogo = memo(({ stlPath = "/models/logo.stl" }: { stlPath?: string }) => {
  ...
  useEffect(() => {
    ...
    // Яркое освещение для цвета #B9DDFF
  ...
  return (
    <div ref={containerRef} className="relative w-full h-80 md:h-96 flex items-center justify-center p-4">
      <div
        ref={mountRef}
        ...
        style={{
          ...
          filter: "drop-shadow(0 10px 20px rgba(13, 43, 82, 0.3))",
        }}
      />

      {!isLoaded && (
        <div className="absolute inset-4 flex items-center justify-center bg-[#B9DDFF] dark:bg-[#1B3644] rounded-lg">
          <div className="flex flex-col items-center space-y-2">
            <div className="w-8 h-8 border-4 border-[#0D2B52] dark:border-[#B9DDFF] border-t-transparent rounded-full animate-spin"></div>
            <p className="text-sm text-black dark:text-white text-center px-2">Загрузка 3D модели...</p>
          </div>
        </div>
      )}

      {error && (
        <div className="absolute bottom-2 left-2 right-2 text-xs text-[#0D2B52] dark:text-[#DEDEBE] text-center px-2">
          {error}
        </div>
      )}
    </div>
  ...
  return (
    <section id="about" className="relative py-20 overflow-hidden">
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-black dark:text-white mb-6">О компании</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center mb-16">
          ...
          >
            <h3 className="text-3xl font-bold text-black dark:text-white mb-6">Наша миссия</h3>
